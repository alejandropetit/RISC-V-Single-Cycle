module datapath #(parameter XLEN = 32)(input logic clk, reset,  SrcACtrl, SrcBCtrl, we, PCSrc, link, input logic [3:0] AluCtrl, input logic [2:0] ExtCtrl, input logic [XLEN-1:0] Instr, output logic [XLEN-1:0] PC, Rsig1, Rsig2);
	
	logic [XLEN-1:0] NextPC, SrcA, SrcB, AluResult, ExtImm, PCp4, toWrite3;
	...
	mux2 #(XLEN) LinkMux(.d0(AluResult),.d1(PCp4),.s(link),.y(toWrite3));
	
	regfile #(XLEN) rf(.r1(Instr[19:15]), .r2(Instr[24:20]), .r3(Instr[11:7]), .s1(Rsig1), .s2(Rsig2), .w3(toWrite3), .clk(clk), .we(we));
	
	extend ext(.Instr(Instr[31:6]), .Ext(ExtImm), .i(ExtCtrl));
